...
**v1.12 progress (2026-06-21):** Phase 85 complete — Aider Polyglot Adapter + 7 Remaining Per-Language Runners. ADAPTER-AIDER-01 + TOOLBENCH-03..09 satisfied; all 4 ROADMAP success criteria verified goal-backward (4/4 must-haves; verifier ran build/test/`make vet`/`make verify-licenses` itself and tamper-tested the license gate). The phase lit up the full language matrix by cloning the Phase-78 `bench/languages/go/runner.go` `LanguageRunner` template ×7 — `bench/languages/{python,java,csharp,cpp,rust,typescript,javascript}/runner.go`, each swapping only Detect/Setup-argv/exec/parser and keeping the load-bearing `Passed = exitCode==0` gate + `ctx.Err()→infra` / `*exec.ExitError→exit-code` split verbatim. Every runner's parser (pytest-json, surefire-XML, TRX, ctest-JUnit, **Rust libtest TEXT** — not `--message-format=json`, Pitfall 2 — vitest/jest-json) is proven by a HERMETIC golden-fixture test as the SOLE authoritative proof (live toolchain runs SKIP cleanly; javac/mvn absent here), defeating the MEMORY false-green trap; the Java golden is REAL-provenance-sourced (apache/maven-surefire@5ee132b4) and guarded by a `TestJavaFixtureProvenance` anti-tautology gate. Three shared gaps closed: an additive `language` open-key on result.v2 (mirroring `embedder_id`) + an aggregator `Report.ByLanguage` per-language slice (SC#1 substrate); a `container.Run(--network=none)` fixed-argv seam on the Phase-84 engine (SC#3, argv-asserted hermetically); and the Aider Polyglot dataset-loader-only adapter (`bench/datasets/aider-polyglot/`: `isHexSHA1`-validated pinned-sha `--depth 1` clone @ 7e0611e7, `.meta/config.json` mapping, 2-attempt stderr-reprompt protocol — live leg confirmed 225 tasks/6 tracks once). SC#4 ships a per-track Exercism `LICENSE-AUDIT.md` (real SPDX + sha256) behind a `make verify-licenses` hard-fail gate cloning the Phase-75 `verify_tos.go` strict-decode discipline. SC#1 full 225-task live run + per-language sanity comparison and SC#3 pre-baked toolchain-image `--network=none` runs are honestly recorded as toolchain/network-gated (decision logic hermetically proven). All 7 plans TDD (RED→GREEN). Code review: 0 blockers, 3 warnings — all fixed with substantive regression tests (WR-01 `restorePristineTests` anti-tamper now wired into the 2-attempt loop so a test-tampering agent can't force a false pass; WR-02 Aider Rust acceptance argv runs `cargo test -- --include-ignored` so `#[ignore]`-gated acceptance tests can't be stubbed past; WR-03 clone dest-leaf guard now propagates `validatePathSegment` to reject `.git`/`.ssh` leaves).

**v1.12 progress (2026-06-21):** Phase 86 complete — CrossCodeEval + RepoBench Adapters + Multi-Oracle Completion Gate. ADAPTER-CCE-01 + ADAPTER-REPO-01 + VERIFIED-03 satisfied; all 4 ROADMAP success criteria verified goal-backward (4/4 must-haves; verifier ran build/test/`make vet`/`make verify-verified-md` itself). RESEARCH caught that the ROADMAP-claimed "Phase 75 HF dataset fetcher" never existed, so this phase BUILT it: a stdlib `net/http` + `apache/arrow-go/v18` pqarrow parquet fetcher (promoted indirect→direct; gomlx/go-huggingface explicitly forbidden), SSRF-pinned (constants-only host/repo, `isHexSHA1` rev pin, `io.LimitReader` 256 MiB cap), path-safe (`validatePathSegment` before `filepath.Join`), cached under `$HELIX_CACHE_DIR` via temp-file+atomic-rename (so a partial write never becomes a cache hit). Three pure leaf scorers (`bench/evaluators/{exactmatch,editsim,identmatch}`) — EM, normalized-Levenshtein edit-similarity (provably NOT the git-numstat `patch_validator.EditDistancePatch`, guarded by a discriminator + import gate), identifier-match — are unit-tested SOLELY against CrossCodeEval paper examples (arXiv:2310.11248). The **multi-oracle completion gate** (VERIFIED-03, the v1.12-specific contribution) requires EM AND edit-similarity≥threshold AND identifier-match all to pass with a per-oracle configurable `ESThreshold`, and produces the EXISTING `evaluators.Metrics.VerifiedCorrectness` additively (no schema bump); its abstain path emits an explicit `verified_correctness=false` (never nil, never a false `true`), documented in `bench/evaluators/VERIFIED.md` behind a `make verify-verified-md` hard gate. Two `dataset-loader-only` adapters land on the aider-polyglot leaf precedent: CrossCodeEval (Py/Java/TS/C#) and RepoBench-R/-C/-P (Py+Java, pinned HF revs, `AccAtK` + EM/ES reuse). A minimal forward-compatible contamination-canary probe (`bench/canary`, the real canary is Phase 89) attaches `canary_contaminated` at score-time + an additive aggregator `CanaryPassRate` column (NULL-disciplined, leaderboard goldens byte-identical), and the `fetch-datasets` CLI stub became a real command. All live HF-fetch + reference-match runs are honestly HELIX_BENCH_NETWORK-gated (committed `sample.parquet` + paper-example fixtures are the sole proof). All 5 plans TDD. Code review found 1 BLOCKER + 4 warnings — **CR-01 was the load-bearing catch: the gate failed OPEN on a zero-value `GateConfig{}` (`DefaultESThreshold` was never applied, so `es >= 0.0` always passed and the 3-oracle AND silently collapsed to 2)** — fixed by flooring a non-positive threshold to 0.9 with a `TestGrade_ZeroValueConfig_ESOracleStillBites` regression; the 4 warnings (RepoBench gold-index decode robustness, int64→int bound check, silent language-mismatch row-drop → explicit error, non-atomic cache write → temp+rename) all fixed and re-verified.
...
---
*Last updated: 2026-06-21 — Phase 86 (CrossCodeEval + RepoBench adapters + multi-oracle completion gate; ADAPTER-CCE-01 + ADAPTER-REPO-01 + VERIFIED-03) complete and verified; v1.12 milestone underway (12/15 phases).*
