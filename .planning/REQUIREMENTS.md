...
- [ ] **INFRA-03**: Eval ↔ bench separation note in `bench/BENCH.md`: `eval/` stays as the in-process PR-gate wiring smoke; `bench/` is the milestone artifact for headline claims (prose-enforced this milestone — no analyzer; D-08). _Acceptance:_ note rendered; reciprocal cross-link added to `eval/EVAL.md` (this one-paragraph pointer is the single permitted change to `eval/` under BENCH-01, which otherwise stays byte-identical).
