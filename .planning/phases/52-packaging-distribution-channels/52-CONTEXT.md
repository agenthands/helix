...
- **D-01:** **Binary renamed `serena` → `helix`.** Hard cut at v1.9. Archive naming template in `.goreleaser.yaml` flips from `serena_v{{ .Version }}_*` to `helix_v{{ .Version }}_*`. Build dir `cmd/serena/main.go` moves to `cmd/helix/main.go`. The Go module path (`github.com/agenthands/helix`) is unchanged. Rationale: user explicitly rejected keeping `serena` as the binary name; `helix` matches the repo and the project's new identity.
...
- **D-13:** **Embed `minisign.pub` via `//go:embed`.** A single Go file (planner picks location; likely `internal/upgrade/pubkey.go` or similar) contains `//go:embed ../../minisign.pub` (or absolute-from-module-root equivalent). Build fails if the file is missing. Same file at repo root continues to be served for the INSTALL.md curl-fetch flow — single source of truth, edited once during key rotation. Hardcoded `const` strings are explicitly NOT used (would require manual sync).
