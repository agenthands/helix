...
This is a TDD plan: tests are written RED first (including the two NEW tests for wrong-identity and wrong-issuer), then GREEN with the sigstore-go verifier, then REFACTOR extracts the identity matcher.
...
Output: A green `go test ./internal/upgrade/...` with eight test cases (six rewritten + two NEW), a single coordinated commit set covering rename + embed + delete (per PATTERNS R-3 ordering hazard), and updated docs.
</objective>
...
Identity-pinning policy (from RESEARCH §"Identity pinning"):
- Issuer: `https://token.actions.githubusercontent.com`
- SAN regex: `^https://github\.com/agenthands/helix/\.github/workflows/release\.yml@refs/tags/v[0-9]+\.[0-9]+\.[0-9]+$`

Canonical error literal (Pitfall-4):
```
signature verification FAILED
```
With inline comment `// canonical: signature verification FAILED — see Pitfall 4.` above every `errors.New(...)` failure return.
</interfaces>
</context>
...
<task type="auto" tdd="true">
  <name>Task 1: RED — write failing tests for the cosign verifier (rewrite + 2 NEW)</name>
  ...
  <read_first>
    ...
    - .planning/phases/58-v1-9-carryover-release-distribution/58-PATTERNS.md §"`internal/upgrade/verify_test.go`" (test-case map: 6 renames + 2 NEW)
    ...
    - .planning/phases/58-v1-9-carryover-release-distribution/58-RESEARCH.md §"Identity pinning" (issuer + SAN regex defaults)
  </read_first>
  <behavior>
    - TestVerifyArchiveHappyPath: a freshly-generated bundle against the test trust root + ephemeral Fulcio-equivalent key for the canonical archive verifies successfully (returns nil).
    ...
    - **NEW** TestVerifyArchiveWrongIdentity: bundle is otherwise valid but its certificate's SAN does NOT match the pinned regex (use a fixture signed for a different repo path) → canonical literal. Maps to T-58-03.
  ...
  <action>
    1. Create `internal/upgrade/testdata/generate_fixtures.go` (build-tag `//go:build ignore`) using sigstore-go's signing API to generate, on demand:
       - `internal/upgrade/testdata/trusted_root.json` (test trust root with ephemeral Fulcio + Rekor + TSA fakes)
       - `internal/upgrade/testdata/sample-archive.tar.gz.sigstore.json` (bundle for the existing `sample-archive.tar.gz` fixture)
       - Adversarial bundles for wrong-identity and wrong-issuer cases generated in `t.TempDir()` rather than checked in (matches existing testify convention from PATTERNS — "Tampered-fixture tests build the tampered file in t.TempDir() rather than checking it in").
    ...
    3. Rewrite `internal/upgrade/verify_test.go` per PATTERNS test-case map:
       - Top-of-file `const` block: keep `canonicalErrText = "signature verification FAILED"`; replace minisign-path consts with `testdataTrustRoot`, `testdataBundle`.
       ...
       - Replace 6 cases per the PATTERNS rename table; ADD `TestVerifyArchiveWrongIdentity` and `TestVerifyArchiveWrongIssuer`; ADD `TestRekorUnreachable`.
       - Per-case assertion uses the existing pattern from PATTERNS lines 215-219 verbatim.
  ...
  <acceptance_criteria>
    ...
    - `internal/upgrade/verify_test.go` contains all 9 test names (`TestVerifyArchiveHappyPath`, `TestVerifyArchiveTamperedBundle`, `TestVerifyArchiveWrongTrustRoot`, `TestVerifyArchiveMissingArchive`, `TestVerifyArchiveMissingBundle`, `TestVerifyArchiveMalformedTrustRoot`, `TestVerifyArchiveWrongIdentity`, `TestVerifyArchiveWrongIssuer`, `TestRekorUnreachable`)
    - `grep -c canonicalErrText internal/upgrade/verify_test.go` returns at least 9 (one assertion per case)
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - .planning/phases/58-v1-9-carryover-release-distribution/58-RESEARCH.md §"Identity pinning" (default issuer + SAN regex)
  ...
  <behavior>
    ...
    - Identity policy pins both OIDC issuer and a SAN regex (from RESEARCH §"Identity pinning").
    ...
    - Every failure branch returns the canonical literal `"signature verification FAILED"` with `// canonical: ...` inline comment (Pitfall-4 invariant).
  ...
  <action>
    ...
    3. **Rewrite `internal/upgrade/verify.go`** with the sigstore-go API:
       ...
       - Define package-level constants:
         ...
         const (
             ...
             // Phase 58 D-02 / R-2: SAN regex pinned to release.yml on a semver tag.
             pinnedSANRegexLiteral = `^https://github\.com/agenthands/helix/\.github/workflows/release\.yml@refs/tags/v[0-9]+\.[0-9]+\.[0-9]+$`
         )
       ...
       - Use `errors.New(...)` (NOT `fmt.Errorf` with `%w`) at every literal-only canonical return; use `fmt.Errorf` ONLY for the Rekor branch where additional structured wording is mandated by D-04.
    ...
    6. Run the test suite. All 9 tests MUST pass:
       ```bash
  ...
  <verify>
    <automated>go test ./internal/upgrade/... -count=1 2&gt;&amp;1 | tee /tmp/58-02-green.log &amp;&amp; grep -qE 'PASS|ok\s+.*internal/upgrade' /tmp/58-02-green.log &amp;&amp; ! grep -q 'FAIL' /tmp/58-02-green.log &amp;&amp; go vet ./internal/upgrade/... &amp;&amp; ! grep -q 'jedisct1/go-minisign' go.mod &amp;&amp; grep -q 'sigstore/sigstore-go' go.mod</automated>
  </verify>
  <acceptance_criteria>
    - All 9 tests in `internal/upgrade/` pass (`go test ./internal/upgrade/... -count=1` returns ok)
    ...
    - The pinned SAN regex literal matches the RESEARCH-default value exactly: `^https://github\.com/agenthands/helix/\.github/workflows/release\.yml@refs/tags/v[0-9]+\.[0-9]+\.[0-9]+$`
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - All 11 tests pass. No behavior change visible to `VerifyArchive` callers.
  </behavior>
  <action>
  ...
  <acceptance_criteria>
    - All 11 tests pass (9 from Task 1 + 2 helper tests)
...
<task type="auto">
  ...
  <action>
    ...
    4. **Update INSTALL.md** (or, if no `INSTALL.md` at the repo root, README.md §Installation). Add a `### Verifying release artifacts` subsection with the user-facing recipe (per RESEARCH Q-3 default):
       ```sh
       cosign verify-blob \
         --bundle helix_v1.10.0_linux_amd64.tar.gz.sigstore.json \
         --certificate-identity-regexp '^https://github\.com/agenthands/helix/\.github/workflows/release\.yml@refs/tags/v[0-9]+\.[0-9]+\.[0-9]+$' \
         --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
         helix_v1.10.0_linux_amd64.tar.gz
       ```
       Use the same regex + issuer values as the verifier's pinned policy (parity is the security invariant).
  ...
  <verify>
    <automated>! test -f minisign.pub &amp;&amp; ! test -f internal/upgrade/minisign.pub &amp;&amp; ! test -f internal/upgrade/testdata/test_keypair.pub &amp;&amp; ! test -f internal/upgrade/testdata/test_keypair.key &amp;&amp; ! test -f internal/upgrade/testdata/sample-archive.tar.gz.minisig &amp;&amp; ! grep -q 'embed-pubkey' Makefile &amp;&amp; grep -q 'update-trust-root' Makefile &amp;&amp; grep -q 'Trust root refresh' CONTRIBUTING.md &amp;&amp; ! grep -q 'MINISIGN_PRIVATE_KEY' CONTRIBUTING.md &amp;&amp; (grep -q 'cosign verify-blob --bundle' INSTALL.md 2&gt;/dev/null || grep -q 'cosign verify-blob --bundle' README.md) &amp;&amp; go build ./... &amp;&amp; go test ./internal/upgrade/... -count=1</automated>
  </verify>
  <acceptance_criteria>
    ...
    - The user-facing cosign recipe lives in `INSTALL.md` OR `README.md` (whichever exists) and matches the regex/issuer values used in `verify.go`
...
| T-58-03 | Spoofing | VerifyArchive accepts attacker-controlled cert with valid Fulcio chain | mitigate | Pinned SAN regex (`^https://github\.com/agenthands/helix/\.github/workflows/release\.yml@refs/tags/v[0-9]+\.[0-9]+\.[0-9]+$`) + pinned OIDC issuer (`https://token.actions.githubusercontent.com`); two NEW unit tests `TestVerifyArchiveWrongIdentity` and `TestVerifyArchiveWrongIssuer` guard regression |
...
<verification>
- All 11 tests pass (`go test ./internal/upgrade/... -count=1`)
- `go vet ./internal/upgrade/...` clean
...
- INSTALL.md / README.md user-facing cosign recipe matches verifier's pinned identity policy
</verification>
...
- The pinned identity policy is byte-identical between `verify.go` and the user-facing INSTALL recipe (security-invariant parity)
...
- Whether INSTALL.md or README.md hosts the user-facing recipe
</output>
