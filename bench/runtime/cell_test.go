...
// NOTE (Phase 83 Task 2): the former TestBaselineRagFailClose — which asserted the
// Phase-80 fail-close (res.Deferred==true, "Phase 83" reason, NO row on disk) — is
// REMOVED. baseline_rag is now a real retrieval-only arm that drives
// cmd/helix-bench-rag and emits a schema-valid result.v2 row carrying embedder_id.
// Its behavior is asserted by TestBaselineRagEmitsRow / TestBaselineRagSameContractBudget
// in baseline_rag_test.go (HELIX_BIN-gated, since the cell spawns the server). The
// generic deferred-outcome DISPATCH mechanism is still exercised below with a
// synthetic injected run (it remains a supported third-outcome the matrix can carry).
