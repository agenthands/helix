...
// violation lives ONLY under testdata/, which the go tool ignores, so
// `make vet` on the real tree stays green.
//
// This is the RED anti-vacuity fixture (T-106-03): deleting the `// want`
// directive below makes TestAnalyzer_RejectsRuntimeImportingTools FAIL, proving
// the analyzer actually fires on a planted runtime → tools/ leak.
