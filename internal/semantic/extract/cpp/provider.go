// Package cppextract is the per-language tree-sitter extraction provider
...
func NewProvider(grammars *treesitter.GrammarRegistry) extract.Provider {
	if grammars == nil {
		panic("cppextract.NewProvider: nil GrammarRegistry")
	}
	g, ok := grammars.GetLanguage("cpp")
	if !ok {
		panic("cppextract.NewProvider: 'cpp' grammar missing from injected GrammarRegistry")
	}
	q, qerr := tree_sitter.NewQuery(g, cppQueries)
	if qerr != nil {
		panic("cppextract.NewProvider: query compile failed: " + qerr.Message)
	}
	return &Provider{grammar: g, query: q}
}

func (p *Provider) Language() string { return "cpp" }
...
func (p *Provider) Queries() string                           { return cppQueries }
...
func (p *Provider) Extract(ctx context.Context, source []byte, file extract.SourceFile) (*extract.ExtractedFile, error) {
	...
	if err := parser.SetLanguage(p.grammar); err != nil {
		return partialFile(file, extract.PartialReasonExtractorBug, fmt.Errorf("set cpp language: %w", err)), nil
	}
	...
	for m := matches.Next(); m != nil; m = matches.Next() {
		...
		for _, c := range m.Captures {
			...
			case cn == "definition.type":
				...
				defNameNode = &node
			// Namespace definitions are structural; skip as symbols
		...
		if refNameNode != nil && refKind != "" {
			...
			// its parent field_expression holds the receiver (operand) in the
			// "argument" field. A bare call (foo()) leaves ReceiverText empty.
			if refKind == "call" {
				if p := refNameNode.Parent(); p != nil && p.Kind() == "field_expression" {
					if recv := p.ChildByFieldName("argument"); recv != nil {
	...
	// HTTP route detection (Crow CROW_ROUTE / cpp-httplib svr.Get).
	out.Routes = detectCppRoutes(*tree.RootNode(), source, file.Path)
	return out, nil
}
...
func (p *Provider) ExtractFile(ctx context.Context, _repoID, path string) (*extract.ExtractedFile, error) {
	...
	if err != nil {
		return partialFile(extract.SourceFile{Path: path, Language: "cpp"}, extract.PartialReasonPermissionDenied, err), nil
	}
	return p.Extract(ctx, source, extract.SourceFile{Path: path, Language: "cpp"})
}
