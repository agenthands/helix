...
// directory under testdata/. For each scenario we read before.cpp, run
// Extract, and compare normalized output against expected.json.
//
// If `-update` is passed, expected.json (and expected_after.json when an
// after fixture exists) are regenerated from the current emit.
...
// source produce byte-identical normalized output.
func TestProvider_Determinism(t *testing.T) {
	registry := testutil.NewTestRegistry(t)
	p := NewProvider(registry).(*Provider)
	src, err := os.ReadFile(filepath.Join(testdataDir, "function_basic", "before.cpp"))
	if err != nil {
		t.Fatalf("read fixture: %v", err)
	}
	a, err := p.Extract(context.Background(), src, extract.SourceFile{Path: "function_basic/before.cpp", Language: "cpp"})
	if err != nil {
		t.Fatalf("Extract A: %v", err)
	}
	b, err := p.Extract(context.Background(), src, extract.SourceFile{Path: "function_basic/before.cpp", Language: "cpp"})
	if err != nil {
