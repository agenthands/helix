...
func TestSmoke_ProviderConstructs(t *testing.T) {
	...
	if p.Language() != "java" {
		t.Fatalf("Language() = %q, want %q", p.Language(), "java")
	}
	if got := p.Extensions(); len(got) != 1 || got[0] != ".java" {
		t.Fatalf("Extensions() = %v, want [.java]", got)
	}
...
func TestSmoke_ExtractsBasicClass(t *testing.T) {
	...
	ef, err := p.Extract(context.Background(), src, extract.SourceFile{Path: "Hello.java", Language: "java"})
	if err != nil {
