// Package kotlinextract is the per-language tree-sitter extraction provider
// for Kotlin source files. Constructed via NewProvider with the
...
func NewProvider(grammars *treesitter.GrammarRegistry) extract.Provider {
	if grammars == nil {
		panic("kotlinextract.NewProvider: nil GrammarRegistry")
	}
	g, ok := grammars.GetLanguage("kotlin")
	if !ok {
		panic("kotlinextract.NewProvider: 'kotlin' grammar missing from injected GrammarRegistry")
	}
	q, qerr := tree_sitter.NewQuery(g, kotlinQueries)
	if qerr != nil {
		panic("kotlinextract.NewProvider: query compile failed: " + qerr.Message)
	}
	return &Provider{grammar: g, query: q}
}

func (p *Provider) Language() string                          { return "kotlin" }
func (p *Provider) Extensions() []string                      { return []string{".kt", ".kts"} }
func (p *Provider) TreeSitterLanguage() *tree_sitter.Language { return p.grammar }
func (p *Provider) Queries() string                           { return kotlinQueries }
...
func (p *Provider) Extract(ctx context.Context, source []byte, file extract.SourceFile) (*extract.ExtractedFile, error) {
	...
	if err := parser.SetLanguage(p.grammar); err != nil {
		return partialFile(file, extract.PartialReasonExtractorBug, fmt.Errorf("set kotlin language: %w", err)), nil
	}
	...
	for m := matches.Next(); m != nil; m = matches.Next() {
		...
		if refNameNode != nil && refKind != "" {
			...
			// Receiver capture for member-call references (obj.foo()):
			// enables CROSS_CALLS edges by matching the receiver to an
			// external import. The captured call name is the method
			// identifier inside a navigation_expression; the receiver is the
			// first named child of that navigation_expression (an identifier
			// for obj.foo(), or a nested navigation_expression for chained
			// a.b.bar()). A bare call (foo()) leaves ReceiverText empty.
			if refKind == "call" {
				if p := refNameNode.Parent(); p != nil && p.Kind() == "navigation_expression" {
					if recv := p.NamedChild(0); recv != nil {
	...
	out.Routes = detectKotlinRoutes(*tree.RootNode(), source, file.Path)
	out.Resources = detectKotlinResources(*tree.RootNode(), source, file.Path)
	return out, nil
}
...
func (p *Provider) ExtractFile(ctx context.Context, _repoID, path string) (*extract.ExtractedFile, error) {
	...
	if err != nil {
		return partialFile(extract.SourceFile{Path: path, Language: "kotlin"}, extract.PartialReasonPermissionDenied, err), nil
	}
	return p.Extract(ctx, source, extract.SourceFile{Path: path, Language: "kotlin"})
}
