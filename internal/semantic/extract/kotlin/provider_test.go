...
func TestProvider_Golden(t *testing.T) {
	...
	if len(scenarios) < 5 {
		t.Fatalf("expected at least 5 scenarios under %s, got %d", testdataDir, len(scenarios))
	}
...
func TestProvider_Determinism(t *testing.T) {
	registry := testutil.NewTestRegistry(t)
	p := NewProvider(registry).(*Provider)
	src, err := os.ReadFile(filepath.Join(testdataDir, "function_basic", "before.kt"))
	if err != nil {
		t.Fatalf("read fixture: %v", err)
	}
	a, err := p.Extract(context.Background(), src, extract.SourceFile{Path: "function_basic/before.kt", Language: "kotlin"})
	if err != nil {
		t.Fatalf("Extract A: %v", err)
	}
	b, err := p.Extract(context.Background(), src, extract.SourceFile{Path: "function_basic/before.kt", Language: "kotlin"})
	if err != nil {
