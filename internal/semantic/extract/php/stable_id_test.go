...
func TestStableID(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.scenario, func(t *testing.T) {
			beforeSrc, err := os.ReadFile(filepath.Join(testdataDir, tc.scenario, "before.php"))
			if err != nil {
				t.Fatalf("read before: %v", err)
			}
			afterSrc, err := os.ReadFile(filepath.Join(testdataDir, tc.scenario, "after.php"))
			if err != nil {
				t.Fatalf("read after: %v", err)
			}
			beforeFile := extract.SourceFile{Path: filepath.Join(tc.scenario, "before.php"), Language: "php"}
			afterFile := extract.SourceFile{Path: filepath.Join(tc.scenario, "after.php"), Language: "php"}
