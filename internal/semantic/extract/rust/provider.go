// Package rustextract is the per-language tree-sitter extraction provider
// for Rust source files. Constructed via NewProvider with the
...
func NewProvider(grammars *treesitter.GrammarRegistry) extract.Provider {
	if grammars == nil {
		panic("rustextract.NewProvider: nil GrammarRegistry")
	}
	g, ok := grammars.GetLanguage("rust")
	if !ok {
		panic("rustextract.NewProvider: 'rust' grammar missing from injected GrammarRegistry")
	}
	q, qerr := tree_sitter.NewQuery(g, rustQueries)
	if qerr != nil {
		panic("rustextract.NewProvider: query compile failed: " + qerr.Message)
	}
	return &Provider{grammar: g, query: q}
}

func (p *Provider) Language() string                     { return "rust" }
...
func (p *Provider) Queries() string                       { return rustQueries }
...
func (p *Provider) Extract(ctx context.Context, source []byte, file extract.SourceFile) (*extract.ExtractedFile, error) {
	...
	if err := parser.SetLanguage(p.grammar); err != nil {
		return partialFile(file, extract.PartialReasonExtractorBug, fmt.Errorf("set rust language: %w", err)), nil
	}
	...
	for m := matches.Next(); m != nil; m = matches.Next() {
		...
		if defNameNode != nil && defKind != "" {
			...
			sf := extract.SymbolFact{
				...
				Visibility:       rustVisibility(name, defNameNode),
		...
		if refNameNode != nil && refKind != "" {
			...
			// Receiver capture for member-call references (obj.method()):
			// enables CROSS_CALLS edges by matching the receiver to an
			// external import. The captured call name is the field_identifier;
			// its parent field_expression holds the receiver in the "value"
			// field. A bare call (foo()) leaves ReceiverText empty.
			if refKind == "call" {
				if p := refNameNode.Parent(); p != nil && p.Kind() == "field_expression" {
					if recv := p.ChildByFieldName("value"); recv != nil {
						rf.ReceiverText = recv.Utf8Text(source)
					}
				}
			}
	...
	out.Routes = detectRustRoutes(*tree.RootNode(), source, file.Path)
	out.Resources = detectRustResources(*tree.RootNode(), source, file.Path)
	return out, nil
}
...
func (p *Provider) ExtractFile(ctx context.Context, _repoID, path string) (*extract.ExtractedFile, error) {
	...
	if err != nil {
		return partialFile(extract.SourceFile{Path: path, Language: "rust"}, extract.PartialReasonPermissionDenied, err), nil
	}
	return p.Extract(ctx, source, extract.SourceFile{Path: path, Language: "rust"})
}
...
func rustVisibility(name string, nameNode *tree_sitter.Node) string {
	if name == "" {
		return "private"
	}
	// Check if the parent definition node has a visibility_modifier (pub).
	if nameNode != nil {
		parent := nameNode.Parent()
		if parent != nil {
			for i := uint(0); i < parent.ChildCount(); i++ {
				if child := parent.Child(i); child != nil && child.Kind() == "visibility_modifier" {
					return "exported"
				}
			}
		}
	}
	// Fallback: Rust types conventionally use PascalCase and are implicitly
	// public when exported from their module, but the pub keyword is the
	// definitive signal.
	if c := name[0]; c >= 'A' && c <= 'Z' {
		return "exported"
	}
