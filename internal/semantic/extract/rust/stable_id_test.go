...
func TestStableID(t *testing.T) {
	...
	cases := []tcase{
		...
		{scenario: "comment_edit_preserves_id", beforeName: "process", afterName: "process", expect: expPreserved},
		{scenario: "body_only_edit_preserves_id", beforeName: "compute", afterName: "compute", expect: expPreserved},
		{scenario: "rename_churns_id", beforeName: "old_name", afterName: "new_name", expect: expChurned},
		{scenario: "signature_change_churns_id", beforeName: "transform", afterName: "transform", expect: expChurned},
	}
	...
	for _, tc := range cases {
		...
		t.Run(tc.scenario, func(t *testing.T) {
			beforeSrc, err := os.ReadFile(filepath.Join(testdataDir, tc.scenario, "before.rs"))
			if err != nil {
				t.Fatalf("read before: %v", err)
			}
			afterSrc, err := os.ReadFile(filepath.Join(testdataDir, tc.scenario, "after.rs"))
			if err != nil {
				t.Fatalf("read after: %v", err)
			}
			beforeFile := extract.SourceFile{Path: filepath.Join(tc.scenario, "before.rs"), Language: "rust"}
			afterFile := extract.SourceFile{Path: filepath.Join(tc.scenario, "after.rs"), Language: "rust"}
