...
// NewResolverWithIndex constructs a C resolver over the given EffectiveReader
...
func (r *Resolver) ResolveChain(ctx context.Context, req types.ChainRequest) (types.ChainResponse, error) {
	...
	// req.ChainTokens[0] takes precedence over re-parsing the (bare)
	// signature; an empty token / empty ChainTokens (e.g. the ResolveSymbol
	// synthetic request) falls back to parseAnnotation.
